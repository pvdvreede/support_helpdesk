  ...
  module Pipeline
    class SupportPipeline < Support::Pipeline::PipelineBase
      def execute
        ...
        if emails.empty?
          raise Support::PipelineProcessingWarn.new "The email has no to or cc email addresses"
        end
        ...
        if support.nil?
          raise Support::PipelineProcessingWarn.new "No support setup exists for any of these email addresses: #{emails.join(", ")}"
        end
        ...
        if !support.search_in_to || !support.search_in_cc
          if support.search_in_to
            unless email.to.to_a.include? support.to_email_address
              raise Support::PipelineProcessingWarn.new "The support email address is not in the to part of the email."
            end
          end

          if support.search_in_cc
            unless email.cc.to_a.include? support.to_email_address
              raise Support::PipelineProcessingWarn.new "The support email address is not in the cc part of the email."
            end
          end
        end
